...
const AdminPages = () => {
  ...
  const saveMutation = useMutation({
    mutationFn: async () => {
      const { error } = await supabase
        .from("pages")
        .upsert({ slug: activeSlug, content: draft as any, updated_at: new Date().toISOString() }, { onConflict: "slug" });
      if (error) throw error;
    },
    onSuccess: () => {
  ...
  const frTrans = (fr as any)[schema.translationKey] as Record<string, string>;
  const enTrans = (en as any)[schema.translationKey] as Record<string, string>;
            ...
            <div key={f.key} className="grid md:grid-cols-2 gap-4 pb-6 border-b border-border last:border-0 last:pb-0">
              <div>
                ...
                <InputComp
                  ...
                  onChange={(e: any) => setField("fr", f.key, e.target.value)}
              ...
              <div>
                ...
                <InputComp
                  ...
                  onChange={(e: any) => setField("en", f.key, e.target.value)}
